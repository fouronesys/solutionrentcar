...
<div class="row">
    ...
    <div class="col-md-4 col-12">
    <label for="price_dop_input" class="col-md-12 col-12 control-label">Monto Pagado (RD$) <span class="text-danger">*</span></label>
         <div class="input-group">
<span class="input-group-text" style="background-color:orange;">RD$</span>
      <input style="background-color:#222;" type="number" step="0.01" min="0" autocomplete="off" required class="form-control" placeholder="Monto Pagado en Pesos" id="price_dop_input">
    </div>
   </div>

    <div class="col-md-4 col-12">
    <label for="price_usd_view" class="col-md-12 col-12 control-label">Equivalente en (USD$)</label>
         <div class="input-group">
<span class="input-group-text" style="background-color:orange;"><i class="fa fa-dollar-sign"></i></span>
      <input style="background-color:#222;" type="text" name="price" autocomplete="off" class="form-control" placeholder="Equivalente en dólares" id="price_usd_view" readonly>
    </div>
   </div>

    <div class="col-md-4 col-12">
    <label for="usd_rate_view" class="col-md-12 col-12 control-label">Tasa Actual del Dólar</label>
         <div class="input-group">
<span class="input-group-text" style="background-color:orange;"><i class="fa fa-line-chart"></i></span>
      <input style="background-color:#222;" type="text" autocomplete="off" class="form-control" placeholder="Cargando tasa..." id="usd_rate_view" readonly>
    </div>
   </div>

    <input type="hidden" name="price" id="price_usd" value="">
    <input type="hidden" name="usd_rate" id="usd_rate" value="">
    <input type="hidden" name="price_dop" id="price_dop" value="">
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var inputDopField = document.getElementById("price_dop_input");
  var inputUsdView  = document.getElementById("price_usd_view");
  var inputRateView = document.getElementById("usd_rate_view");

  var inputUsd      = document.getElementById("price_usd");
  var inputRate     = document.getElementById("usd_rate");
  var inputDop      = document.getElementById("price_dop");
  ...
  function recalcularUSD() {
    var dop = limpiarNumero(inputDopField.value);

    if (dop <= 0 || tasaActual <= 0) {
      inputUsdView.value = "";
      inputUsd.value = "";
      inputDop.value = "";
      return;
    ...
    var usd = dop / tasaActual;

    inputUsdView.value = formatoNumero(usd);
    inputUsd.value = usd.toFixed(2);
    inputDop.value = dop.toFixed(2);

    inputRateView.value = formatoNumero(tasaActual);
    inputRate.value = tasaActual.toFixed(4);
  }

  async function cargarTasaDolar() {
    try {
      inputRateView.value = "Cargando...";
      inputUsdView.value = "";

      const response = await fetch("https://api.frankfurter.dev/v2/rates?base=USD&quotes=DOP");
      const data = await response.json();

      if (Array.isArray(data) && data.length > 0 && typeof data[0].rate !== "undefined") {
        tasaActual = parseFloat(data[0].rate);
      } else if (data && data.rates && typeof data.rates.DOP !== "undefined") {
        tasaActual = parseFloat(data.rates.DOP);
      }

      if (!tasaActual || isNaN(tasaActual)) {
        inputRateView.value = "No disponible";
        inputRate.value = "";
        return;
      }
      ...
      console.error("Error obteniendo tasa Forex:", error);
      inputRateView.value = "No disponible";
      inputRate.value = "";
      inputUsdView.value = "";
      inputUsd.value = "";
  ...
  inputDopField.addEventListener("input", function(){
    recalcularUSD();
  });
