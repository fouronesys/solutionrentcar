...
<div class="row">
 ...
 <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
           <h1 class="m-0"><i class='fa fa-street-view'></i> Listado de Marcas</h1>
          </div><!-- /.col -->
...
<form class="form-horizontal" method="post" id="add" role="form">
  <div class="row">
    <div class="col-md-8 col-8">
      ...
      <input style="background-color:#333;" type="text" name="name" autocomplete="off" class="form-control" placeholder="Nombre de la Marca" required>
    </div>
    <div class="col-md-4 col-4">
...
<script>
// Guardar marcas localmente si no hay internet
function guardarMarcaOffline(marca) {
  let marcas = JSON.parse(localStorage.getItem("marcas_pendientes")) || [];
  marcas.push(marca);
  localStorage.setItem("marcas_pendientes", JSON.stringify(marcas));
  toastr.info('Guardado localmente. Se enviará cuando vuelva el internet.');
}

// Sincronizar marcas cuando vuelva la conexión
function sincronizarMarcas() {
  let marcas = JSON.parse(localStorage.getItem("marcas_pendientes")) || [];
  if (marcas.length > 0 && navigator.onLine) {
    marcas.forEach((marca, i) => {
      fetch("./?action=brands&opt=add_offline", {
  method: "POST",
...
document.getElementById("add").addEventListener("submit", function(e) {
  e.preventDefault();
  const form = this;
  const marca = { name: form.name.value };

  if (navigator.onLine) {
    fetch("./?action=brands&opt=add", {
      method: "POST",
    ...
    .then(resp => {
      if (resp.trim() === "OK") {
       toastr.success('Registro agregado correctamente.');
        window.location = './?view=brands&opt=all';
      } else {
         toastr.error('Ya existe ese registro.');
      }
    })
    .catch(() => guardarMarcaOffline(marca));
  } else {
    guardarMarcaOffline(marca);
  }
});

// Intentar sincronizar cada 5 segundos
setInterval(() => {
  if (navigator.onLine) sincronizarMarcas();
}, 5000);
</script>
...
$base = new Database();
$con = $base->connect();
$sql = "select SQL_BIG_RESULT * from brand";
$query = $con->query($sql);
    if(count($query)>0):?>
<div class="card"  style="background-color:#222;">
<div class="card-body">
        ...
        <tr>
                  <td class="text-right py-0 align-middle">
                      <div class="btn-group btn-group-sm btn-block">
                        <a href="./?view=brands&opt=all&id=<?php echo $r['id'];?>" class="btn btn-warning"><i class="fas fa-edit"></i></a>
                      </div>
        </td>
...
if ($x['permits_id']==4): ?>
                        ...
                        <a href="./?action=brands&opt=del&id=<?php echo $r['id'];?>" class="btn btn-danger btn-block btn-sm" onclick="return confirmDelete();" ><i class="fas fa-trash"> Eliminar</i></a>
