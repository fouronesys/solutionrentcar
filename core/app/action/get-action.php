...
while($r = $query->fetch_array()){ $cars = CarsData::getById($r["id"]); $provider = SuppliersData::getById($cars->provider_id);
  $html.="<option value='".$cars->id."' data-description='".strtoupper($cars->getStock()->name)."'>".$cars->getBrand()->name." ".$cars->name." ".$cars->year." ".$cars->getExColor()->name." [".$cars->chassis."]</option>";
  }
  echo $html; 
endif; 
