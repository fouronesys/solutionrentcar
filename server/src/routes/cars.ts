...
carsRouter.get("/:id(\\d+)", h(async (req, res) => {
  const company = await resolveCompany(req, res);
  if (!company) return;
  const car = await one("SELECT * FROM cars WHERE id=$1 AND company_id=$2", [toInt(req.params.id), company.id]);
  if (!car) return err(res, "not_found", "Vehículo no encontrado", 404);
...
carsRouter.get("/", h(async (req, res) => {
  ...
  if (req.query.q) {
    ...
    where.push(`(name ILIKE $${vals.length} OR year ILIKE $${vals.length} OR plate ILIKE $${vals.length})`);
  }
  ...
  const r = await q(
    `SELECT * FROM cars WHERE ${where.join(" AND ")} ORDER BY id DESC LIMIT $${vals.length - 1} OFFSET $${vals.length}`,
    vals,
  );
  return ok(res, {
