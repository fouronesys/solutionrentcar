...
- El puerto 21 puede funcionar desde el workspace cuando se usa la IP de hPanel; si el hostname FTP da `NXDOMAIN`, no asumir fallo de credenciales: resolver la IP publicada por hPanel y probar lectura.
**Why:** el hostname FTP configurado dejó de resolver, pero la misma cuenta sí leyó `/DEALER` usando la IP del servidor.
