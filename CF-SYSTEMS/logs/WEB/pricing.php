...
$base = new Database();
$con = $base->connect();
$sql = "select SQL_BIG_RESULT * from cars where status<>1 and stock_id=".$selstock;
$query = $con->query($sql);
if(count($query)>0): 
...
while($r = $query->fetch_array()): $cars = CarsData::getById($r["id"]);?>   
						      ...
						      <tr>
						        ...
						        <td class="price" style="color:white; background-color:#CFD1D1;">
						        	<p class="btn-custom"><a href="booking?id=<?php echo $r['id'];?>&single=day" style="color:white; background-color:rgb(<?php echo $color[0];?>, <?php echo $color[1];?>,<?php echo $color[2];?>);">Book Now</a></p>
						       ...
						       <td class="price" style="color:white; background-color:#CFD1D1;">
						        	<p class="btn-custom"><a href="booking?id=<?php echo $r['id'];?>&single=month" style="color:white; background-color:rgb(<?php echo $color[0];?>, <?php echo $color[1];?>,<?php echo $color[2];?>);">Book Now</a></p>
						        	<div class="price-rate">
							        	<h3>
							        		<span class="num" style="color:rgb(<?php echo $color[0];?>, <?php echo $color[1];?>,<?php echo $color[2];?>);"><small class="currency">$</small> <?php echo $r['price'] * 30 * (1 - 0.05); ?></span>
							        		<span class="per" style="color:black;">/per month</span>
							        	</h3>
							        	<span class="subheading" style="color:black;">1/4 fuel upon delivery</span>
						        </div>
						        </td>
