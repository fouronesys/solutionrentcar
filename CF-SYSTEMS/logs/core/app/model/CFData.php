<?php
class CFData {
	public static $tablename = "cfinal";
	...
	public static function getById($id){
		...
		return Model::one($query[0],new CFData());
	}
	...
	public static function getAllProductsBySellId($sell_id){
		...
		$query = Executor::doit($sql);
		return Model::many($query[0],new CFData());
	}

	public static function getAll(){
		$sql = "select SQL_BIG_RESULT * from ".self::$tablename." where status!='Usado' limit 1";
		$query = Executor::doit($sql);
		return Model::many($query[0],new CFData());
	}


	public static function getAllByNC(){
		$sql = "select name_in from ".self::$tablename." where status!='Usado' order by id desc limit 1";
		$query = Executor::doit($sql);
		return Model::many($query[0],new CFData());
	}
	...
	public function update(){
		$sql = "update ".self::$tablename." set sell_id=\"$this->sell_id\", status=\"$this->status\" where id=\"$this->cf_id\"";
		Executor::doit($sql);
	}
