<?php
class CRSData {
	public static $tablename = "crspecial";
	...
	public static function getById($id){
		...
		return Model::one($query[0],new CRSData());
	}

	public static function getAll(){
		$sql = "select SQL_BIG_RESULT * from ".self::$tablename." where status='' limit 1";
		$query = Executor::doit($sql);
		return Model::many($query[0],new CRSData());
	}
	...
	public function update(){
		$sql = "update ".self::$tablename." set sell_id=\"$this->sell_id\", status=\"$this->status\" where id=\"$this->crs_id\"";
		Executor::doit($sql);
	}
