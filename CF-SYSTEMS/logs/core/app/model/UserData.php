...
class UserData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (language,no,comision,name,lastname,firma,email,image,kind,stock_id,password,status,created_at) ";
		$sql .= "value (\"$this->language\",\"$this->no\",\"$this->comision\",\"$this->name\",\"$this->lastname\",\"$this->firma\",\"$this->email\",\"$this->image\",\"$this->kind\",\"$this->stock_id\",\"$this->password\",\"$this->status\",$this->created_at)";
		Executor::doit($sql);
	}
...
// partiendo de que ya tenemos creado un objecto UserData previamente utilizamos el contexto
	public function update(){
		$sql = "update ".self::$tablename." set language=\"$this->language\",no=\"$this->no\",comision=\"$this->comision\",firma=\"$this->firma\",name=\"$this->name\",email=\"$this->email\",lastname=\"$this->lastname\",image=\"$this->image\",status=\"$this->status\" where id=$this->id";
		Executor::doit($sql);
	}
