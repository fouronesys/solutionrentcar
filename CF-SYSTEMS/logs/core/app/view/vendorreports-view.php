...
<div class="row">
	...
	<div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
           <h1 class="m-0"><i class="fa fa-copy"></i> Reportes <small>[Vendedores Populares]</small></h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item "><i class='far fa-circle'></i> Vendedores Populares</a></li>
...
<?php if (Core::$user->kind==1):?>
	<select style="background-color: #222;" name="stock_id" class="form-control select2">
...
<?php else:?>
<select style="background-color: #33;" name="stock_id" class="form-control select2">
...
<?php if($_GET["sd"]!="" && $_GET["ed"]!=""):?>
			...
			$operations = array();
      if ($_GET["stock_id"]!="") {
      		$operations = BookingData::getSQL("select SQL_BIG_RESULT *,sum(total) as st from booking where date(created_at) >= \"$_GET[sd]\" and date(created_at) <= \"$_GET[ed]\" and status=3 and stock_id=\"$_GET[stock_id]\" group by user_id order by st desc");
      }else{
			$operations = BookingData::getSQL("select SQL_BIG_RESULT *,sum(total) as st from booking where date(created_at) >= \"$_GET[sd]\" and date(created_at) <= \"$_GET[ed]\" and status=3 and stock_id=\"$_GET[stock_id]\" group by user_id order by st desc");
		}?>
