...
<div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
           <h1 class="m-0"><i class="fa fa-copy"></i> Reportes de Mantenimientos
          </div><!-- /.col -->
...
actualizarReloj(); // Llamada inicial
            ...
            <form>
            <input type="hidden" name="view" value="maintenancereport">
<div class="row">
...
<select style="background-color: #333;" name="client_id" class="form-control select2">
  ...
  <option value="<?php echo $p->id;?>"><?php echo $p->getBrand()->name." ".$p->name." ".$p->year." ".$p->getExColor()->name;?></option>
  <?php endforeach; ?>
</select>
...
<?php if($_GET["sd"]!=""&&$_GET["ed"]!=""):?>
      ...
      $operations = MaintenanceData::getAllByDateOp($_GET["sd"],$_GET["ed"],StockData::getPrincipal()->id);
      }
      else if($_GET["client_id"]=="" && $_GET["user_id"]!=""){
      $operations = MaintenanceData::getAllByDateOpByUserId($_GET["user_id"],$_GET["sd"],$_GET["ed"],StockData::getPrincipal()->id);
      }
      else if($_GET["client_id"]!="" && $_GET["user_id"]==""){
      $operations = MaintenanceData::getAllByDateBCOp($_GET["client_id"],$_GET["sd"],$_GET["ed"],StockData::getPrincipal()->id);
      }else{
      $operations = MaintenanceData::getAllByDateBCOpByUserId($_GET["user_id"],$_GET["client_id"],$_GET["sd"],$_GET["ed"],StockData::getPrincipal()->id);
      } 
