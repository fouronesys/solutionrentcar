...
$sheet->setCellValue('A1', 'Reporte de Vendedores Populares')
->setCellValue('A2', 'Id Vendedor')
->setCellValue('B2', 'Total Venta')
->setCellValue('C2', 'Vendedor');

$start = 3;
foreach($operations as $operation){
$sheet->setCellValue('A'.$start, $operation->user_id)
->setCellValue('B'.$start, $symbol." ".number_format($operation->st,2,".",","))
->setCellValue('C'.$start, $operation->getUser()->name." ".$operation->getUser()->lastname);
$start++;
}
...
header('Content-Disposition: attachment;filename="topvendor-'.time().'.xlsx"');
