      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <Card>
          <View style={styles.row}>
            <Text style={styles.title}>{car?.brand ? `${car.brand} ` : ""}{car?.name ?? car?.model ?? `#${booking.id}`}</Text>
        ...
        <Card>
          ...
          {[0, 1].includes(status) && (
            <>
              <Button title={t("booking.deliver")} onPress={() => confirmAndCall(t("booking.confirmDeliver"), () => api.post(`/bookings/${id}/deliver`))} loading={acting} style={{ marginBottom: 8 }} />
              <Button title={t("booking.cancel")} variant="danger" onPress={() => confirmAndCall(t("booking.confirmCancel"), () => api.post(`/bookings/${id}/cancel`))} loading={acting} style={{ marginBottom: 8 }} />
            </>
          )}
          {status === 3 && (
            <Button title={t("booking.return")} onPress={() => confirmAndCall(t("booking.confirmReturn"), () => api.post(`/bookings/${id}/return`))} loading={acting} style={{ marginBottom: 8 }} />
          )}
          <Button title={t("booking.registerPayment")} variant="secondary" onPress={() => router.push({ pathname: "/(staff)/pay/[bookingId]", params: { bookingId: String(id) } })} />
        </Card>
