  ...
  const load = useCallback(async () => {
    ...
    try {
      const params: Record<string, string | number | undefined> = {
        status: 0, q: q || undefined, limit: 50,
      };
      if (filtered) {
          ...
          return (
            ...
            >
              ...
              <Text style={styles.meta}>
                {item.year ? `${item.year} · ` : ""}{item.transmission ?? ""}{item.fuel ? ` · ${item.fuel}` : ""}
              </Text>
...
const styles = StyleSheet.create({
  ...
  dateBtn: {
    ...
    padding: 10, marginHorizontal: 3, backgroundColor: "#fff",
  },
