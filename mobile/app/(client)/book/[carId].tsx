 ...
 * dark RUTA card, price summary and notes as accordions,
 * visual range calendar instead of native pickers, toast for feedback.
 */
...
export default function BookScreen() {
  ...
  const [start, setStart] = useState<Date>(() =>
    paramStart ? new Date(paramStart) : defaultStart(),
  );
  ...
  const registerGuest = async (): Promise<boolean> => {
    if (!gName.trim()) {
      toast({ message: t("book.guestNameRequired"), type: "error" });
      return false;
    }
    if (!gPhone.trim()) {
      toast({ message: t("book.guestPhoneRequired"), type: "error" });
      return false;
    }
    if (!gPassword.trim()) {
      toast({ message: t("book.guestPasswordRequired"), type: "error" });
      return false;
    }
    if (gPassword.length < 6) {
      toast({ message: t("book.guestPasswordShort"), type: "error" });
      return false;
    }
    if (
      gPassword.replace(/\D/g, "") === gPhone.replace(/\D/g, "") &&
      gPhone.replace(/\D/g, "") !== ""
    ) {
      toast({ message: t("book.guestPasswordSameAsPhone"), type: "error" });
      return false;
    }
    ...
    try {
      ...
      if (e instanceof ApiError) {
        ...
        if (phoneTaken) {
          toast({ message: t("book.guestPhoneTaken"), type: "error", duration: 5000 });
        } else {
          toast({ message: e.message || t("book.guestRegisterError"), type: "error" });
        }
      } else {
        toast({ message: t("book.guestRegisterError"), type: "error" });
      }
      return false;
    }
    return true;
  ...
  const submit = async () => {
    ...
    try {
      ...
      const r = await api.post<{ booking_id: number }>("/bookings", {
        ...
        place_end: sameReturn
          ? placeStart.trim()
          : placeEnd.trim() || placeStart.trim(),
        comment: comment.trim() || undefined,
      });
      toast({ message: t("book.success"), type: "success" });
      setTimeout(() => {
        if (r.booking_id) {
          router.replace({
            pathname: "/(client)/booking/[id]",
            params: { id: String(r.booking_id), justBooked: "1" },
          });
        } else {
      ...
      >
        ...
        >
          ...
          <View style={styles.rutaCard}>
            ...
            {/* ── Date range row — opens visual calendar ── */}
            <Pressable
              style={styles.rutaDatesRow}
              onPress={() => setCalendarOpen(true)}
            >
              <View style={styles.rutaDateCell}>
                <Ionicons
                ...
                />
                <View>
                  <Text style={styles.rutaDateLabel}>
                    {locale === "en" ? "From" : "Desde"}
                  </Text>
                  <Text style={styles.rutaDateValue}>
                    {fmtShort(start)}, {fmtTime(start)}
                  </Text>
                </View>
              </View>
            ...
            <View style={styles.carCard}>
              ...
              <View style={{ flex: 1 }}>
                ...
                {[
                  ...
                  car.transmission,
                  car.seat
                    ? `${car.seat} ${locale === "en" ? "seats" : "pasajeros"}`
                    : null,
                ]
                  .filter(Boolean)
                  .join(" · ") ? (
          ...
          <View
            onLayout={(e) => {
              placesY.current = Math.max(
                0,
                e.nativeEvent.layout.y - 12,
              );
            }}
          >
            <Card style={styles.section} elevation="sm">
              ...
              >
                ...
                <Text style={styles.sameReturnText}>
                  {t("book.sameReturn")}
                </Text>
              </Pressable>
          ...
          {/* ── Price summary — accordion (collapsed) ────────────────── */}
          <Accordion
            title={locale === "en" ? "PRICE SUMMARY" : "RESUMEN DE PRECIO"}
            ...
            <View style={styles.summaryInner}>
              <View style={styles.summaryRow}>
                <Text style={styles.summaryRowLabel}>
                  {locale === "en" ? "Rental" : "Alquiler"} · {days}{" "}
                  {locale === "en" ? "days" : "días"}
                </Text>
                <Text style={styles.summaryRowAmount}>{money(subtotal)}</Text>
              </View>
              <View style={styles.summaryRow}>
                <Text style={styles.summaryRowLabel}>
                  {locale === "en"
            ...
            <>
              <Card style={styles.section} elevation="sm">
                ...
                <View style={styles.nameRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Input
                      label={t("book.guestLastname")}
                      value={gLastname}
                      onChangeText={setGLastname}
                      autoCapitalize="words"
                      icon="people-outline"
                    />
                  </View>
              ...
              <View style={styles.guestInfoCard}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={styles.guestInfoTitle}>
                    {t("book.guestInfoTitle")}
                  </Text>
                  <Text style={styles.guestInfoBody}>
                    {t("book.guestInfoBody")}
                  </Text>
                  <Pressable
                    onPress={goToLogin}
                    hitSlop={6}
                  ...
                  >
                    <Text style={styles.guestLoginText}>
                      {t("book.guestHaveAccount")}{" "}
                      <Text style={styles.guestLoginTextBold}>
                        {t("book.guestSignIn")}
                      </Text>
                    </Text>
                  </Pressable>
...
const makeStyles = () =>
  StyleSheet.create({
    ...
    // Price summary (inside accordion)
    summaryInner: { paddingTop: 4 },
    summaryRow: {
      flexDirection: "row",
      justifyContent: "space-between",
      alignItems: "center",
